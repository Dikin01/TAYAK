...
const string eMarkPath = @"../../../test.txt";
...
void InitTags(XmlElement xmlElement, TagBase parent)
{
    ...
    foreach (var item in xmlElement.ChildNodes)
    {
        ...
        if (item is XmlElement nextXmlElement)
        {
            ...
            {
                ColumnTag column = new( width ?? defaultWidth, height ?? parent.Height)
                {
            ...
            {
                RowTag row = new(width ?? parent.Width, height ?? defaultHeight)
                {
                    Valign = valign ?? Valign.Top,
                    Halign = halign ?? Halign.Left,
                    BgColor = bgColor ?? 0,
            ...
            {
                BlockTag block = new(width ?? parent.Width, height ?? defaultHeight)
                {
                    Columns = columns ?? 1,
                    Rows = rows ?? 0,
                };
...
void PrintTag(TagBase parent, int startColumn, int startRow)
{
